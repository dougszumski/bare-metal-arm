...
static inline USB_BDT *bdt_rx(int num) {
	return &bdt[num * BDT_PER_EP];
}
static inline USB_BDT *bdt_tx(int num) {
	return bdt_rx(num) + (BDT_PER_EP / 2);
}
...
enum {
	POWER, ENUMERATED, ENABLED, ADDRESS, READY
};
static int device_state;
static uint8_t device_address;

static const USB_DEV_DSC device_descriptor = { .bLength = sizeof(USB_DEV_DSC),
		.bDscType = mDEVICE, .bcdUSB = 0x0200, .bDevCls = 0x02, .bDevSubCls =
				0x00, .bDevProtocol = 0x00, .bMaxPktSize0 = EP0_BUFSIZE,
		.idVendor = 0xDEAD, .idProduct = 0xBEAF, .bcdDevice = 0x0000, .iMFR =
				0x01,         // Index to string Manufacturer descriptor
...
static const USB_CONFIG config_descriptor = { { .bLength = sizeof(USB_CFG_DSC),
		.bDscType = mCONFIGURATION, .wTotalLength = sizeof(USB_CONFIG),
		.bNumIntf = 2, .bCfgValue = 1, .iCfg = 0, .bmAttributes = 0xC0,
		.bMaxPower = 0x32 }, { .bLength = sizeof(USB_INTF_DSC), .bDscType =
		mINTERFACE, .bIntfNum = CDC_STATUS_INTERFACE, .bAltSetting = 0,
		.bNumEPs = 1, .bIntfCls = 0x02, .bIntfSubCls = 0x02, .bIntfProtocol = 1,
		.iIntf = 0 }, { .bFNLength = sizeof(USB_CDC_HEADER_FN_DSC), .bDscType =
		0x24, .bDscSubType = 0x00, .bcdCDC = 0x0110 }, { .bFNLength =
		sizeof(USB_CDC_CALL_MGT_FN_DSC), .bDscType = 0x24, .bDscSubType = 0x01,
		.bmCapabilities = 0x00, .bDataInterface = 1 }, { .bFNLength =
		sizeof(USB_CDC_ACM_FN_DSC), .bDscType = 0x24, .bDscSubType = 0x02,
		.bmCapabilities = 0x06 }, { .bFNLength = sizeof(USB_CDC_UNION_FN_DSC),
		.bDscType = 0x24, .bDscSubType = 0x06, .bMasterIntf =
				CDC_STATUS_INTERFACE, .bSaveIntf0 = CDC_DATA_INTERFACE }, {
		.bLength = sizeof(USB_EP_DSC), .bDscType = mENDPOINT, .bEPAdr =
				CDC_ACM_ENDPOINT | 0x80, .bmAttributes = 0x03, .wMaxPktSize =
				CDC_ACM_SIZE, .bInterval = 64 }, { .bLength =
		sizeof(USB_INTF_DSC), .bDscType = mINTERFACE, .bIntfNum =
		CDC_DATA_INTERFACE, .bAltSetting = 0, .bNumEPs = 2, .bIntfCls = 0x0A,
		.bIntfSubCls = 0x00, .bIntfProtocol = 0, .iIntf = 0 }, { .bLength =
		sizeof(USB_EP_DSC), .bDscType = mENDPOINT, .bEPAdr = CDC_RX_ENDPOINT,
		.bmAttributes = 0x02, .wMaxPktSize = CDC_RX_SIZE, .bInterval = 0 }, {
		.bLength = sizeof(USB_EP_DSC), .bDscType = mENDPOINT, .bEPAdr =
				CDC_TX_ENDPOINT | 0x80, .bmAttributes = 0x02, .wMaxPktSize =
				CDC_TX_SIZE, .bInterval = 0 } };
...
static const usb_string manufacturer =
		USB_STRING("A\0P\0 \0C\0o\0n\0s\0u\0l\0t\0i\0n\0g\0 \0L\0L\0C\0");
static const usb_string product = USB_STRING("H\0A\0C\0K\0");
static const usb_string serial = USB_STRING("1\0");

const uint8_t string0[4] = { 0x04,          // bLength
...
static const usb_descriptor_list_t usb_descriptor_list[] = { { 0x0100,
		(uint8_t *) &device_descriptor, sizeof(device_descriptor) }, { 0x0200,
		(uint8_t *) &config_descriptor, sizeof(config_descriptor) },

// Strings
		{ 0x0300, (uint8_t *) string0, 0 }, { 0x0301, (uint8_t *) &manufacturer,
				0 }, { 0x0302, (uint8_t *) &product, 0 }, { 0x0303,
				(uint8_t *) &serial, 0 }, { 0x0000, 0, 0 },       // End marker
		};
...
static void usb_handler(uint8_t stat) {
	...
	case SETUP_TOKEN:
		...
		case 0:
			usb_setup_device(ep, setup);
			break;
		case 1:
			usb_setup_interface(ep, setup);
			break;
		case 2:
			usb_setup_endpoint(ep, setup);
			break;
		default:
			break;
		}
